...
The pip command installs the searchio package with the MCP server from
PyPI. The curl command adds the Rust engine sidecar at
~/.searchio/bin/se-serve (linux, macOS, Windows with Git Bash). The
sidecar is the fast tier-2 backend. It is optional but recommended.
...
Note. `npx @ioserviceslabs/searchio-mcp` also works as the client
command in step 3. The launcher starts the Docker image when Docker
runs. Without Docker it starts the local install from step 2.
## 1. Install
...
`searchio doctor` checks the install. The Rust engine sidecar is
optional. Install it with the curl command in the quick start, step 2.
